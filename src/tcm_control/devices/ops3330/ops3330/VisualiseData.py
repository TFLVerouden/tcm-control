...
# # Read the data
# data = []
# data.append(ParticleData(directory='./',
#             suffix='OPS', oversize_bin_cutoff=12.4))


def calculate_average_and_std(data, output_dir):
    ...
    # Show plot
    plt.tight_layout()
    plt.savefig(output_dir / "count_vs_diameter_plot.png", dpi=300)
    plt.savefig(output_dir / "count_vs_diameter_plot.pdf", dpi=300)
...
def calculate_average_and_std_volume(data, output_dir):
    ...
    # Show plot
    plt.tight_layout()
    plt.savefig(output_dir / "volume_vs_diameter_plot.png", dpi=300)
    plt.savefig(output_dir / "volume_vs_diameter_plot.pdf", dpi=300)
    # plt.show()
...
def plot_counts_over_time(data, output_dir):
    ...
    for d in data:
        ...
        if d.get_count_over_time().shape[0] != len(time_points):
            print(
                f"Warning: Time points mismatch for suffix {d.suffix}. Expected {len(time_points)}, got {d.get_count_over_time().shape[0]}.")
    ...
    plt.figure(figsize=(7, 5))
    plt.errorbar(time_points, average_total, yerr=std_total, fmt='o--',
                 markerfacecolor='white', markersize=8, color='skyblue', label='Total Count', capsize=5)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "count_vs_time_plot.png", dpi=300)
    plt.savefig(output_dir / "count_vs_time_plot.pdf", dpi=300)
    # plt.show()


def OPS_visualize_data(output_dir: Path) -> None:
    # Read the data
    data = []
    data.append(ParticleData(directory=output_dir,
                suffix='OPS', oversize_bin_cutoff=12.4))

    calculate_average_and_std(data, output_dir)
    calculate_average_and_std_volume(data, output_dir)
    plot_counts_over_time(data, output_dir)


if __name__ == "__main__":
    OPS_visualize_data(output_dir=Path("./data"))
