...
def _time_test_length(test_length: Optional[float]) -> str:
    """Convert test length in seconds to a string in the format D:H:M:S."""
    if test_length is None:
        return ""
    test_length_i = int(round(test_length))
    days, remainder = divmod(test_length_i, 86400)
    hours, remainder = divmod(remainder, 3600)
    minutes, seconds = divmod(remainder, 60)
    return f"{days}:{hours}:{minutes}:{seconds}"


def _time_sample_interval(sample_interval: float) -> str:
    """Convert sample interval in seconds to a string in the format H:M:S."""
    sample_interval_i = int(round(sample_interval))
    hours, remainder = divmod(sample_interval_i, 3600)
    minutes, seconds = divmod(remainder, 60)
...
def _hms_to_seconds(time_str: str) -> int:
    """Convert H:M:S string to integer seconds."""
    hours, minutes, seconds = time_str.split(":")
    return int(hours) * 3600 + int(minutes) * 60 + int(seconds)


class OPS:
    """High-level OPS helper with method-based measurement recording."""

    def __init__(self, client: OPSClient):
        self.client = client
    ...
    def start_standard_recording(
        self,
        profile_toml_path: str | Path,
        out_csv_path: str | Path,
        extra_duration_s: float = 10.0,
        poll_interval_divisor: float = 4.0,
        start: bool = True,
        stop_when_done: bool = True,
    ) -> None:
        """
        Standardized entry point: apply profile TOML and start background logging.

        This is intended to keep calling code short: provide only profile TOML
        and output CSV path, while duration/poll interval are derived from the
        profile logging settings.
        """
        from .profile import load_profile, apply_profile

        profile = load_profile(profile_toml_path)
        apply_profile(self.client, profile)
        ...
        max_duration_s = (
            ...
            + repeat_interval_s
            + float(extra_duration_s)
        )
        poll_interval_s = sample_interval_s / float(poll_interval_divisor)

        self.record_measurement_async(
    ...
    def record_measurement_async(
        ...
        def _runner() -> None:
            try:
                self._record_result = self.record_measurement(
                    out_path=out_path,
        ...
        self._record_thread = threading.Thread(
            target=_runner,
            name="ops3330-record-measurement",
            daemon=True,
        )
        self._record_thread.start()

    def is_recording(self) -> bool:
        """Return True while a background OPS recording is active."""
        return self._record_thread is not None and self._record_thread.is_alive()

    def collect_recording(self, timeout: Optional[float] = None) -> Path:
        """Wait for background recording to finish and return the CSV path."""
        if self._record_thread is None:
            raise RuntimeError("No background OPS recording has been started.")
    ...
    def record_measurement(
        ...
        model = self.client.read_model_number()
        serial = self.client.read_serial_number()
        firmware = self.client.read_firmware_version()
        cal_date = self.client.read_calibration_date()
        ch_setup = self.client.read_channel_setup()
        log_setup = self.client.read_log_setup()
        alarm_setup = self.client.read_alarm_setup()
        user_cal_setup = self.client.read_user_cal()
        flow_cal_setup = self.client.read_flow_cal()
        ...
        with out_path.open("w", newline="") as f:
            ...
            try:
                while True:
                    ...
                    try:
                        logged = self.client.read_logged_bins()
                    ...
                    valid_flag = meta[0] if meta else None
...
def record_measurement(
    ...
    """Backward-compatible wrapper around OPS.record_measurement()."""
    return OPS(client).record_measurement(
        out_path=out_path,
        max_samples=max_samples,
        max_duration_s=max_duration_s,
        poll_interval_s=poll_interval_s,
