...
def plot_single_dataset_accuracy(ds, plot_config, colors, goal_df=None, goal_flow_col=None, goal_label='Goal flow', output_pdf='simple_flowcurve.pdf'):
    """
        Plot ONE dataset showing:
            - Input flow rate (left axis)
    ...
    fig, ax1 = plt.subplots(1, 1, figsize=(5.0, 4.0), constrained_layout=False)
    # Keep fixed margins so axes/labels do not shift between export variants.
    fig.subplots_adjust(left=0.14, right=0.86, bottom=0.14, top=0.96)
    set_grid(ax1, mode="vertical", on=False)

    # ax1.plot(t_s, q_lps, color=colors[0])

    show_input = plot_config.get('show_input', True)
    show_measured = plot_config.get('show_measured', True)

    line_measured = None
    if show_measured:
        # Draw measured first and with lower z-order so input stays in front.
        line_measured, = ax1.plot(
            piv_df['time_s'], piv_df[flow_col], color=colors[4],
            linewidth=2.5, zorder=1, label='Measured\nusing PIV'
        )

    # Goal flow: prefer external goal file if provided, otherwise skip.
    line_goal = None
    if show_input and goal_df is not None and goal_flow_col is not None:
        line_goal, = ax1.plot(goal_df['time_s'], goal_df[goal_flow_col],
                              linewidth=2.5, linestyle='--', color=colors[0], alpha=1, zorder=3,
                              label=goal_label)
    ax1.set_ylabel("Flow rate (L/s)")

    # # Gupta model
    # MODEL_GENDER = "Male"
    # MODEL_WEIGHT_KG = 71.0
    # MODEL_HEIGHT_M = 1.94
    ...
    # )
    # t_model_s, q_model_lps = cough.flow(time_s=goal_df['time_s'], units="L/s")
    # t_model_s = np.asarray(t_model_s, dtype=float)
    # q_model_lps = np.asarray(q_model_lps, dtype=float)

    # ax1.plot(t_model_s, q_model_lps, linewidth=3,
    #          linestyle="--", color="#a9cba0")
    ...
    # add_label(ax1, "Input", xy=(0.20, 3), ha="right",
    #           coord_system="data", color="black")
    # add_label(ax1, "Measured", xy=(0.13, 6),
    #           coord_system="data", color="#2ba02b")

    legend_handles = []
    legend_labels = []
    if line_measured is not None:
        legend_handles.append(line_measured)
        legend_labels.append('Measured\nusing PIV')
    if line_goal is not None:
        legend_handles.append(line_goal)
        legend_labels.append('Input based\non literature*')
    if legend_handles:
        ax1.legend(legend_handles, legend_labels,
                   loc='upper right', bbox_to_anchor=(0.50, 0.98, 0.48, 0.01),
                   mode='expand', borderaxespad=0.0)
        if len(legend_handles) > 1:
            reorder_legend(ax1, [1, 0],
                           loc='upper right', bbox_to_anchor=(0.50, 0.98, 0.48, 0.01),
                           mode='expand', borderaxespad=0.0)
    set_ticks_every(ax1, axis="x", step=0.1)
    set_ticks_every(ax1, axis="y", step=2)
...
def export_measured_render_video(ds,
                                 colors,
                                 goal_df,
                                 goal_flow_col,
                                 output_video,
                                 ...
                                 duration_s=0.350):
    """
    Export a video where the measured PIV line is progressively revealed.
    The reveal timing follows recorded time (record_fps), while playback runs at playback_fps.
    """
    ...
    # Static input curve
    ax1.plot(goal_df['time_s'], goal_df[goal_flow_col],
             linewidth=2.5, linestyle='--', color=colors[0], alpha=1, zorder=3,
             label='Input based\non literature*')

    # Measured curve to be progressively revealed
    measured_line, = ax1.plot([], [], color=colors[4],
                              linewidth=2.5, zorder=1, label='Measured\nusing PIV')
    ...
    legend_handles = [measured_line]
    legend_labels = ['Measured\nusing PIV']
    legend_handles.append(ax1.lines[0])
    legend_labels.append('Input based\non literature*')
    ax1.legend(legend_handles, legend_labels,
               loc='upper right', bbox_to_anchor=(0.50, 0.98, 0.48, 0.01),
               mode='expand', borderaxespad=0.0)
    ...
    # Restrict measured reveal to the first duration_s of recorded time.
    piv_mask = (piv_df['time_s'] >= 0.0) & (piv_df['time_s'] <= duration_s)
    piv_t = piv_df.loc[piv_mask, 'time_s'].to_numpy(dtype=float)
    piv_q = piv_df.loc[piv_mask, flow_col].to_numpy(dtype=float)

    if len(piv_t) == 0:
        print("No measured points in the requested video time range; skipping video export.")
        plt.close(fig)
        return

    n_frames = max(1, int(round(duration_s * record_fps)))
    ...
    def update(frame_idx):
        t_visible = frame_idx / float(record_fps)
        visible = piv_t <= t_visible
        measured_line.set_data(piv_t[visible], piv_q[visible])
        return (measured_line,)
    ...
    anim = animation.FuncAnimation(
        ...
        init_func=init,
        frames=n_frames,
        interval=1000.0 / float(playback_fps),
        blit=True,
        repeat=False
    )

    out_video = FilePath(output_video)
    out_video.parent.mkdir(parents=True, exist_ok=True)

    try:
        from tqdm import tqdm
        writer = animation.FFMpegWriter(fps=playback_fps, bitrate=1800)
        with tqdm(total=n_frames, desc="Rendering video", unit="frame") as pbar:
            anim.save(out_video, writer=writer,
                      savefig_kwargs={"facecolor": "black"},
                      progress_callback=lambda i, n: pbar.update(1))
        print(f"Saved: {out_video}")
    except Exception as e:
        print(f"Could not export MP4 video ({e}).")
        print("Install ffmpeg and ensure it is available on PATH to enable MP4 export.")
    finally:
        plt.close(fig)
...
def main():
    ...
    goal_file = "/Users/tommieverouden/Library/CloudStorage/OneDrive-UniversityofTwente/Teaching & supervision/2025-11 Tijn/Random/Plot Tijn/goal file piv format.csv"
    ...
    for i in range(n_datasets):
        ...
        piv_file = "/Users/tommieverouden/Library/CloudStorage/OneDrive-UniversityofTwente/Teaching & supervision/2025-11 Tijn/Random/Plot Tijn/flow_rate.csv"
        # if not piv_file:
        ...
        valve_file = "/Users/tommieverouden/Library/CloudStorage/OneDrive-UniversityofTwente/Teaching & supervision/2025-11 Tijn/Random/Plot Tijn/Test1CalibratedCurve_20260128-115204.csv"
        if not valve_file:
    ...
    colors = use_tcm_poster_style(dark_mode=True, black_white_first=True)

    # Export 1: input only
    plot_single_dataset_accuracy(
        datasets[idx],
        {
            'show_input': True,
            'show_measured': False,
        ...
        colors=colors,
        goal_df=goal_df,
        goal_flow_col=goal_flow_col,
        goal_label='Input based\non literature*',
        output_pdf='temp/output2/simple_flowcurve2_1.pdf'
    )

    # Export 2: input + measured
    plot_single_dataset_accuracy(
        datasets[idx],
        {
            'show_input': True,
        ...
        goal_flow_col=goal_flow_col,
        goal_label='Input based\non literature*',
        output_pdf='temp/output2/simple_flowcurve2_2.pdf'
    )

    # Export video: measured curve progressively rendered over first 350 ms
    export_measured_render_video(
        datasets[idx],
        colors=colors,
        goal_df=goal_df,
        goal_flow_col=goal_flow_col,
        output_video='temp/output2/simple_flowcurve2_measured_render.mp4',
        record_fps=20000,
        playback_fps=60,
        duration_s=0.350
    )
