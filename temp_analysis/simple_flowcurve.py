...
def plot_single_dataset_accuracy(ds, plot_config, colors, goal_df=None, goal_flow_col=None, goal_label='Goal flow', output_pdf='simple_flowcurve.pdf'):
    ...
    show_setpoint = plot_config.get('show_setpoint', False)
    ...
    if show_measured:
        ...
        line_measured, = ax1.plot(
            piv_df['time_s'], piv_df[flow_col], color=colors[-2],
            linewidth=2.5, zorder=1, label=f'{label} - PIV measured'
        )
    ...
    # Always create and style the secondary axis so layout stays identical.
    ax2 = ax1.twinx()
    ax2.set_ylim(12, 20)
    if show_setpoint:
        ax2.set_ylabel('Setpoint (mA)', color=colors[4])
        ax2.tick_params(axis='y', labelcolor=colors[4])
    else:
        transparent = (0, 0, 0, 0)
        ax2.set_ylabel('Setpoint (mA)', color=transparent)
        ax2.tick_params(axis='y', colors=transparent, labelcolor=transparent)

    line_setpoint = None
    if show_setpoint:
        valve_df_filtered = valve_df[valve_df[setpoint_col] != -1]
        if not valve_df_filtered.empty:
            line_setpoint, = ax2.plot(
                valve_df_filtered['time_s'],
                valve_df_filtered[setpoint_col],
                color=colors[4],
                linewidth=2.5,
                linestyle=':',
    ...
    if line_setpoint is not None:
        legend_handles.append(line_setpoint)
        legend_labels.append('Valve setting')
    if legend_handles:
...
def main():
    ...
    plot_single_dataset_accuracy(
        ...
        goal_label='Goal flow (external)',
        output_pdf='simple_flowcurve1_dark.pdf'
    )

    # Export 2: input + setpoint
    plot_single_dataset_accuracy(
        datasets[idx],
        {
            'show_input': True,
        ...
        goal_flow_col=goal_flow_col,
        goal_label='Goal flow (external)',
        output_pdf='simple_flowcurve2_dark.pdf'
    )

    # Export 3: input + setpoint + measured (measured drawn behind input)
    plot_single_dataset_accuracy(
        datasets[idx],
        {
            'show_input': True,
        ...
        goal_flow_col=goal_flow_col,
        goal_label='Goal flow (external)',
        output_pdf='simple_flowcurve3_dark.pdf'
    )
